...
# Small Problems Medium 1 - Problem 4: Passing Parameters Part 2
# Fri. 01/08/2021
...
category names. Can we do that without extracting items directly from the array based on their indices?
...
block variables in such a way that it ignores the first two elements, and groups all
remaining elements as a raptors array.

=end

def find_raptors(arr)
  yield(arr)
end


birds = %w(raven finch hawk eagle)
p find_raptors(birds) { |_, _, *remaining_birds| raptors = remaining_birds   }
