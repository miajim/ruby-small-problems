...
# Small Problems Easy 1 - Problem 7: True for All?
# Thurs. 01/07/21

=begin
In the previous exercise, you developed a method called any? that is similar to
the standard Enumerable#any? method for Arrays (our actual solution works with
any Enumerable collection). In this exercise, you will develop its companion, all?.
...
true for every element, then #all? returns true. Otherwise, #all? returns false.
Note in particular that #all? will stop searching the collection the first time the block returns false.
...
def all?(arr)
  arr.each do |elem|
    return false unless yield(elem) # return !true if !yield(elem)
  end
  true # !false
end
