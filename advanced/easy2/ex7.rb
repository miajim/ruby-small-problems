...
# Small Problems Easy 2 - Problem 7: each_with_object
# Thurs. 01/07/2021

=begin
The Enumerable#each_with_object method iterates over the members of a collection,
passing each element and an arbitrary object (usually a collection) to the associated block.
The value returned by the block is not used. The object that is passed to the block
is defined as an argument to each_with_object; with each iteration, this object may be updated by the block.
...
def each_with_object(arr, collection)
  arr.each { |elem| yield(elem, collection) }
  collection
end
