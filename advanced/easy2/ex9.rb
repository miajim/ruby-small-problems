...
# Small Problems Easy 2 - Problem 9: each_cons(Part 1)
# Fri. 01/08/2021

=begin
The Enumerable#each_cons method iterates over the members of a collection taking
each sequence of n consecutive elements at a time and passing them to the associated
block for processing. It then returns a value of nil.
...
while, or until to iterate through the Array passed in as an argument, but must not
...
def each_cons(arr)
  ...
  previous_elem = arr[0]
  arr[1..-1].each do |current_elem|
    yield(previous_elem, current_elem)
